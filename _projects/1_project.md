...
## Predicting Onflow Parameters for Domain and Task Adaptation
 ...
 - A transfer learning framework utilizing ConvNet based architectures is proposed for domain and task adaptation.
...
{% cite yilmaz25 %}
 ...
 - With the goal of first bringing the knowledge learned from computational fluid dynamics (CFD) data to the wind tunnel testing phase with streaming data, a transfer learning approach is proposed and demonstrated for a wind turbine blade tip.
 - This approach requires first training a neural network offline by using only CFD data, then freezing the weights of the selected layers, and retraining the remaining layers using only wind tunnel data.
